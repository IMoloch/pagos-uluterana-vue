<template>
    <div v-if="isOpen" class="modal-overlay">
      <div class="modal-container bg-white p-5 rounded-lg relative shadow-lg max-w-md w-full">
        ...
        <div class="modal-header p-4 text-center flex items-center justify-center relative" style="background-color: #10143C; border-radius: 8px 8px 0 0;">
          <h2 class="text-white font-bold text-lg">
            {{ isEditMode ? 'Actualizar Tarjeta' : 'Agregar Tarjeta' }}
          </h2>
          <!-- Botón de cierre centrado en la franja -->
          <button class="close-button absolute right-4 text-white hover:text-gray-300" @click="$emit('close')">✖</button>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4 mt-4">
          ...
          <div class="form-group">
            ...
            <input 
              ...
              class="input input-bordered w-full" 
            />
            <small v-if="errors.expirationDate" class="text-red-500">{{ errors.expirationDate }}</small>
          </div>
          ...
          <button type="submit" :class="isEditMode ? 'btn btn-primary w-full' : 'btn btn-success w-full'">
            {{ isEditMode ? 'ACTUALIZAR TARJETA' : 'AGREGAR TARJETA' }}
          </button>
        </form>
  
        <!-- Botón para eliminar tarjeta en modo edición -->
        <button v-if="isEditMode" @click="handleDelete" class="btn btn-error w-full mt-4">ELIMINAR TARJETA</button>
      </div>
    </div>
  </template>
  ...
  const props = defineProps({
    isOpen: Boolean,
    card: Object, // Tarjeta seleccionada (si existe)
  });
  
  const emit = defineEmits(['close', 'save', 'delete']);
  
  const cardNumber = ref(props.card ? props.card.number : '');
  const cardOwner = ref(props.card ? props.card.name : '');
  const expirationDate = ref(props.card ? props.card.expDate : '');
  const cvv = ref('');
  ...
  const formatCardNumber = () => {
    ...
    cardNumber.value = cardNumber.value.replace(/\s+/g, '').replace(/(.{4})/g, '$1 ').trim();
    validateCardNumber();
  };
  ...
  const validateCVV = () => {
    errors.value.cvv = /^\d{3}$/.test(cvv.value)
      ? '' 
